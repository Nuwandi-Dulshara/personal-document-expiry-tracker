...
public interface IExpiryService { (string Status, int DaysRemaining) Calculate(DateOnly expiryDate, DateOnly? today = null); }
...
{
    Task<IReadOnlyList<NotificationStateDto>> GetAsync(int userId);
    Task<(bool Success, string? Error)> MarkReadAsync(int userId, MarkNotificationsReadDto request);
}
